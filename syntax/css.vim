...
" Last Change:    2012 Apr 03
" CSS2 by Nikolai Weibull
" Full CSS2, HTML4 support by Yeti
" Some CSS3 and HTML5 support added by Arno Renevier
...
syn keyword cssTagName abbr acronym address applet area audio a b base
syn keyword cssTagName basefont bdo big blockquote body br button canvas
...
syn keyword cssTagName source span strike strong style sub sup tbody td
syn keyword cssTagName textarea tfoot th thead title tr tt ul u var video
...
syn match cssValueAbsoluteLength contained "[-+]\=\d\+\(\.\d*\)\=\(em\|ex\|px\|gd\|rem\|vw\|vh\|vm\|ch\)"
syn match cssValueRelativeLength contained "[-+]\=\d\+\(\.\d*\)\=\(mm\|cm\|in\|pt\|pc\)"
syn match cssValuePercentage contained "[-+]\=\d\+\(\.\d*\)\=\(%\)"
syn match cssValueAngle contained "[-+]\=\d\+\(\.\d*\)\=\(deg\|grad\|rad\|turn\)"
...
syn match cssColorProp contained "\<background\(-\(color\|image\|repeat\|attachment\|position\|clip\|origin\|size\)\)\=\>"
syn keyword cssColorAttr contained center scroll fixed
syn keyword cssColorAttr contained space round
...
syn match cssTextProp "\<\(\(word\|letter\)-spacing\|text\(-\(decoration\|transform\|align\|justify\|index\|shadow\)\)\=\|vertical-align\|unicode-bidi\|line-height\)\>"
...
syn keyword cssTextAttr contained capitalize uppercase lowercase center justify baseline sub super start end
...
syn match cssTransitionAttr contained "\<linear\|ease\(-\(in-out\|in\|out\)\)\|cubic-bezier\>"
...
syn match cssRenderProp contained "\<\(display\|marker-offset\|unicode-bidi\|white-space\(-collapse\)\=\|list-item\|run-in\|inline-table\|word-break\|\(text\|word\)-wrap\)\>"
...
syn keyword cssRenderAttr contained unrestricted suppress
syn match cssRenderAttr contained "\<bidi-override\>"
syn match cssRenderAttr contained "\<discard\|\(preserve\|word\)\(-break\)\=\|keep-all\|loose\|break-\(strict\|all\|word\)\>"
...
syn region cssNamespace start="@namespace" end=";" contains=cssStringQ, cssStringQQ, cssUnicodeEscape, cssComment, cssURL
...
syn keyword cssPseudoClassId contained link visited active hover focus before after left right root empty target enabled disabled checked warning indeterminate
...
if version >= 508 || !exists("did_css_syn_inits")
  ...
  HiLink cssPseudoClassNot Constant
  HiLink cssValueAbsoluteLength Number
  HiLink cssValueRelativeLength Number
